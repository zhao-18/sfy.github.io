var CSSReload = {
  ...
  _listenToPostMessages: function () {
    ...
    window[this._eventMethod()](this._messageEvent(), function (t) {
      try {
        if (!e._shouldHandleMessage(t)) return;
  ...
  _refreshCSS: function (e) {
    ...
    s.type = "text/css", s.className = "timestyle", s.styleSheet ? s.styleSheet.cssText = e.css : s.appendChild(document.createTextNode(e.css)), this.head.appendChild(s), t && t.parentNode.removeChild(t), "prefixfree" === e.css_prefix && StyleFix.process()
  },
  _findPrevCPStyle: function () {
    for (var e = document.getElementsByTagName("style"), t = e.length - 1; t >= 0; t--)
      if ("timestyle" === e[t].className) return e[t];
    return !1
  }
};
CSSReload.init();
